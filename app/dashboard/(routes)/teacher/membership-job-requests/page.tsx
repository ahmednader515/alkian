...
export default function MembershipJobRequestsPage() {
  ...
  const fetchReservations = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get("/api/reservations?type=MEMBERSHIP");
      console.log("Reservations response:", response.data);
      setReservations(response.data.reservations || []);
    } catch (error: any) {
      console.error("Error fetching reservations:", error);
      toast.error(error?.response?.data?.error || "حدث خطأ أثناء جلب الطلبات");
      setReservations([]);
    } finally {
      setIsLoading(false);
    }
  ...
  // Extract previous experience from message
  const getPreviousExperience = (message?: string) => {
    if (!message) return null;
    const match = message.match(/الخبرات السابقة:\s*(.+?)(?:\n|$)/);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl md:text-3xl font-bold text-gray-900 dark:text-white">
            طلبات العضوية والوظيفة
          </h1>
          <p className="text-muted-foreground mt-2">
            إدارة طلبات العضوية وطلب الوظيفة
          </p>
        </div>
      ...
      {filteredReservations.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground text-center">
              {filter === "ALL" 
                ? "لم يتم إرسال أي طلبات عضوية أو وظيفة بعد"
        ...
        <div className="grid gap-6">
          {filteredReservations.map((reservation) => {
            ...
            const previousExperience = getPreviousExperience(reservation.message);
            ...
            return (
              <Card key={reservation.id} className="hover:shadow-lg transition-shadow">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {reservation.governorate || "غير محدد"}
                        </p>
                ...
                <CardContent className="space-y-4">
                  ...
                  {reservation.message && !previousExperience && (
