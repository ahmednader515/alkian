...
export default function OnlineCourseRegistrationsPage() {
  ...
  const fetchReservations = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get("/api/reservations?type=ONLINE_COURSE");
      console.log("Reservations response:", response.data);
      setReservations(response.data.reservations || []);
    } catch (error: any) {
      console.error("Error fetching reservations:", error);
      toast.error(error?.response?.data?.error || "حدث خطأ أثناء جلب طلبات التسجيل");
      setReservations([]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            إدارة طلبات التسجيل في الكورسات الأونلاين
          </p>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground text-center">
              {filter === "ALL" 
                ? "لم يتم إرسال أي طلبات تسجيل في الكورسات بعد"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {reservation.course?.title || "كورس غير محدد"}
